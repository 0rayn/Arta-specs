...
In Arta's machine syntax, every verb must have a Subject Prefix (`[m.]`, `[n.]`, `[k.]`). To prevent these prefixes from being misread as standalone nouns or verbs, Tarbits use the physical Decimal Point (DP) located at the bottom-right of the display housing as a Syntax Modifier. When the DP is violently punched or drilled out next to a letter, it casts that letter as the executing Subject.
...
- *[tʼ] (Sharp Switch):* #glyph(b: true, c: true, f: true, e: true) (A fast vertical dual-line structure).
- *[ʃ] (Static):* #glyph(a: true, g: true, d: true) (Three parallel horizontal lines). Looks exactly like a radio wave interference symbol.
- *[qʼ] (Heavy Breaker):* #glyph(b: true, c: true, g: true) (A heavy box structure). It takes force to carve, mirroring the heavy throat-pop.
...
- *[ə] (Neutral State):* #glyph(g: true) (Just the middle bar). Voltage is baseline.
- *[u] (Low State):* #glyph(d: true) (Just the bottom bar). Voltage is grounded.

== 4. Field Execution

If a Tarbit needs to carve the command `[m.tʼ i ʃ]` ("I boot the system") into a blast door, they don't write letters. They execute the script:
